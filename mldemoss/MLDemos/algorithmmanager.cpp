...
// DEBUG include
#include "kmeans.h"
#include "C:\Users\Guillaume\Desktop\MLDemos development\GitMLDemos\mldemos\_AlgorithmsPlugins\KernelMethods\clustererKM.h"
#include "C:\Users\Guillaume\Desktop\MLDemos development\GitMLDemos\mldemos\_AlgorithmsPlugins\OpenCV\classifierBoost.h"
    ...
    FOR(i, trainSamples.size())
    {
        ...
        else
        {
            if(bMulticlass)
            {
                ...
                {
                    // Bug could be here for Boosting Real
...
void AlgorithmManager::ClusterOptimize()
{
    ...
    for(int k=startCount; k<=stopCount; k++)
    {
        ...
        FOR(j, crossValCount)
        {
            ...
            float logLikelihood = clusterer->GetLogLikelihood(samples);
            std::cout<< "just after getlikelihod LogLik: " << logLikelihood << std::endl;
            float BIC           = 0;
            float AIC           = 0;

            if(dynamic_cast<ClustererKM*>(clusterer)){
                std::cout<< "doing K-means clustering" << std::endl;
                AIC = logLikelihood + k * samples[0].size();
                BIC = logLikelihood + k *logf(samples.size()); // BIC
           }else{
                BIC = -2*logLikelihood + k*logf(samples.size())*clusterer->GetParameterCount(); // BIC
                AIC = -2*logLikelihood + 2*clusterer->GetParameterCount(); // AIC
            }
            ...
            std::cout<< " loglik: " << logLikelihood << std::endl;
