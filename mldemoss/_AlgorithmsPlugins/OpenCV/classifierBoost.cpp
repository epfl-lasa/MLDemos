...
float ClassifierBoost::Test( const fvec &sample, fvec *responses) const
{
    ...
    // allocate memory for weak learner output
     float y; // not used variable;
    if(boostType == 0){
        y = model->predict(x, NULL, weakResponses); // with CV_WHOLE_SEQ it craches for boostType ==0
    }else{
        y = model->predict(x, NULL, weakResponses, CV_WHOLE_SEQ);
    }
